...
export const remove = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const friendships = await ctx.db.query("friends").withIndex("by_conversationId", q => q.eq("conversationId", args.conversationId)).unique();
        if (!friendships) {
            throw new ConvexError("Friend ould not be found");
        }
