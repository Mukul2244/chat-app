...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (requestAlreadyReceived) {
            throw new ConvexError("This user has already sent a request to you");
        }
        ...
        if (friend1.some(f => f.user2 === receiver._id) || friend2.some(f => f.user1 === receiver._id)) {
...
export const reject = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!request || request.receiver !== currentUser._id) {
            throw new ConvexError("There was an error rejecting the request");
        }
